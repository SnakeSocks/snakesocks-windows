...
binary_safe_string client_make_auth_query(const connect_info *conn)
{
    /*bstr = malloc();
...
bool client_deal_auth_reply(const connect_info *conn, binary_safe_string server_reply) //Return true if auth is ok.
{
    //return isok(server_reply);
    return server_reply.str[0];
}
binary_safe_string client_encode(const connect_info *conn, client_query payload)
{
...
client_query client_decode(const connect_info *conn, binary_safe_string data)
{
    /*if(somecond)
    {
...
binary_safe_string server_encode(const connect_info *conn, client_query payload)
{
    /*if(somecond)
    {
        return do_something(data);
...
client_query server_decode(const connect_info *conn, binary_safe_string data)
{
    /*if(somecond)
    {
        return do_something(data);
